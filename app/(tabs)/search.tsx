...
export default function SearchScreen() {
  ...
  const [isTabletLayout, setIsTabletLayout] = useState(Platform.OS === 'web' && Platform.isPad);
  ...
  // Update layout on window resize (web only)
  useEffect(() => {
    if (Platform.OS === 'web') {
      const handleResize = () => {
        setIsTabletLayout(window.innerWidth >= 768);
      };
    ...
    <View style={styles.container}>
      ...
      {showingSearchResults ? (
        ...
        <FlatList
          ...
          columnWrapperStyle={isTabletLayout ? styles.columnWrapper : undefined}
        />
      )}
    </View>
  );
